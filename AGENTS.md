...
- **三档 AI 难度**：
  ...
  - **中等（蒙特卡洛）**：模拟落子打分（攻守平衡评估，见 AI 算法表），取最高分；但对"怎么走都差不离"的步（top3 分差 <15）做**带权随机次优**，让开局/平稳期有变化，关键手不受影响（见「次优随机化」说明）。
...
- **平局规则**：双方连续两次 Pass（`passCount>=2`）且无人吃满 5 子 → 平局，播放 `平局.wav`。`hasLegalMove()` 检测是否存在合法落子（含劫/自杀排除），AI 无合法落子时主动 Pass 触发终局，避免"空点全为自杀/劫点"时 AI 乱落子或无限对停。**停一手（Pass）明显提示**：AI 或玩家停一手时均弹「停一手」提示框（"AI 选择停一手（Pass）…双方连续停一手则平局结束本局"），防止用户误以为 AI 卡死/错失回合；连续 Pass 判平局时由平局弹框承担，不重复打扰。
...
- **关键手保护（AI 时刻有 1 气濒死块时）**：`GoCore::whiteInAtari()` 返回盘上是否存在 1 气白块；`weakSaveLevel(r,c)` 返回该落点对最弱 1 气块的补救档位（0 无济 / 1 缓解 / 2 救活）。UI 用它：
  - `doHeuristicMove`（中等）**关闭次优随机**（`!core.whiteInAtari()` 才随机），保证关键时刻必走最优；
  - `doHybridMove`（困难）把候选**收窄为"能救它"的点**再做 MC，防止模拟胜率噪声把救援点翻案成攻击点。
  实测困难救援局面稳定 picks 救援点、空盘开局仍有 3 种起手。
- **放弃（Pass）决策与提示**：`doHeuristicMove` / `doHybridMove` 在全局最佳（综合）评估分 ≤0 时直接 `pass()`——收官阶段"怎么走都亏"时不乱填送子。仅空点全负时才触发，不影响正常对局。**Pass 必须明显提示**：AI 停一手时弹「停一手」提示框（"AI 选择停一手（Pass）。双方连续停一手则平局结束本局。"），玩家按停一手同样弹框；双方连续 Pass 判平局时由平局弹框承担。避免用户以为 AI 卡死/错失回合。
- **次优随机化（游戏性）**：`doHeuristicMove`（中等）取评估分 top3，当 `best − second < 15` 判定"这一步不关键"，则按权重 `best − 自身分 + 1` 加权随机挑一着；分差 ≥15 的关键手（救援 +80、提子 +100、送死 −150 等）**必选最优**，棋力不损、对局不重复。实测空盘首手有 3 种起手，救援局面 50/50 恒选救援点；困难因 MC wins/4 天然带变化，未额外加随机。
...
| 中等 | 中等 (蒙特卡洛) | 每个空点用 `assessMove` 攻守平衡打分，top3 分差<15 时带权随机次优 | O(n²) | 攻守兼备；次优随机化避免每局重复 |
...
| **进攻-连接** | 每相邻白子 +12 | 报团成势 |
| **己方濒死块营救** | 盘上最弱白块≤2气：救活 +块大小×80 / 缓解 +×40；**仅当最弱块为 1 气**、**且存在可救点**而本手不救则 −块大小×40 | 逃跑**优先于**截断/连接（权重 80 可压过 3/4 号项的攻击分）。**2 气弱块不设放任罚**；**1 气块若全盘没有任何点能救（已被围死）也不罚**——否则"救不了、全盘为一张块陪罚"会把整盘打成负分、误触发 Pass 弃棋（实测：2气×3子块整盘 max=−108；1气×2子围死块使开阔点也 −32~−108，`rescuePossible` 判定后 max 恢复 +128，AI 恢复落子） |
...
- 实现位于 `GoCore::assessMove()`：先对落点四邻收集**敌方黑块落子前的气**（`enemyBlocks`），在临时副本 `sim` 落子后对比气差求挤压量，再统计相邻黑/白子数。**己方逃命是全局的**：落子前全盘扫出最弱白块（气最小），并判定 `rescuePossible`（该 1 气块是否至少存在一个空点能把它气 +1）——只有"能救而不救"的点吃放任罚，才能把最高分让给真实救援点。
- **中等与困难共用该评分**（困难的蒙特卡洛候选筛选第一步也用它），因此两档棋力同时增强。
- **困难选点 = 评估分主导 + 模拟胜率/4 细调**（`doHybridMove` 第 3 步）：蒙特卡洛用**双方纯随机落子** playout，噪声很大——权重过高会被"远离战场、随机对局不易被抓的虚高胜率点"带偏，导致困难反而弱于纯评估的中等（实测：wins 权重 1:1 时困难 5:7、中等 8:4）。故取 `评估分 + wins(0~100)/4` 综合最大者：评估分差 1 点胜过 4 点胜率，只影响分数接近的点。实测该权重下困难 7:5、中等 6:6（vs 固定入门黑策略，12 局抽样）。**新增送死角惩罚后棋力小幅再升**：中等 7:5、困难 9:3；罚分提至 150 后抽样：中等 11:1、困难 8:4。加入 Pass 决策、次优随机化、关键手保护与 rescuePossible 修正后再抽样：中等 10:2、困难 9:3（随机化/修正不影响救援与棋力，属抽样波动）。
...
![](./开发不易，感谢支持zfb.jpg)**更新日期**：2026-09-08  
**版本**：1.0（第一版定稿)